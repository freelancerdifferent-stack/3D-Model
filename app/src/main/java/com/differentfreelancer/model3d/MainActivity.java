...
public class MainActivity extends Activity {
    ...
    private void saveProjectJson(String project,String json){runOnUiThread(()->{try{Uri tree=projectTree();if(tree==null){Toast.makeText(this,"Pilih folder project dulu",Toast.LENGTH_LONG).show();chooseProjectFolder();return;}Uri dir=ensureProjectDir(tree,project);if(dir==null)throw new Exception("Folder project gagal dibuat");Uri file=upsertFile(dir,"project.json","application/json");if(file==null)throw new Exception("project.json gagal dibuat");try(OutputStream out=getContentResolver().openOutputStream(file,"wt")){if(out==null)throw new Exception("Output tidak tersedia");out.write(json.getBytes(StandardCharsets.UTF_8));}Toast.makeText(this,"Project tersimpan: "+safeName(project),Toast.LENGTH_SHORT).show();webView.evaluateJavascript("window.onProjectFolderChosen&&window.onProjectFolderChosen("+quoteJs(safeName(project))+")",null);}catch(Exception e){Toast.makeText(this,"Save project gagal: "+e.getMessage(),Toast.LENGTH_LONG).show();}});}
    private String quoteJs(String s){return "\""+s.replace("\\","\\\\").replace("\"","\\\"").replace("\n","\\n")+"\"";}

    @Override protected void onActivityResult(int requestCode,int resultCode,Intent data){super.onActivityResult(requestCode,resultCode,data);if(requestCode==PROJECT_FOLDER_REQUEST){if(resultCode==RESULT_OK&&data!=null&&data.getData()!=null){Uri uri=data.getData();try{getContentResolver().takePersistableUriPermission(uri,Intent.FLAG_GRANT_READ_URI_PERMISSION|Intent.FLAG_GRANT_WRITE_URI_PERMISSION);}catch(Exception ignored){}prefs().edit().putString(PREF_PROJECT_TREE,uri.toString()).apply();webView.evaluateJavascript("window.onProjectFolderChosen&&window.onProjectFolderChosen('Folder selected')",null);}else webView.evaluateJavascript("window.onProjectFolderError&&window.onProjectFolderError('Pemilihan folder dibatalkan')",null);enableImmersiveFullscreen();return;}if(requestCode==FILE_CHOOSER_REQUEST&&fileCallback!=null){Uri[] result=null;if(resultCode==RESULT_OK&&data!=null&&data.getData()!=null){Uri uri=data.getData();result=new Uri[]{uri};try{int flags=data.getFlags()&(Intent.FLAG_GRANT_READ_URI_PERMISSION|Intent.FLAG_GRANT_WRITE_URI_PERMISSION);if((flags&Intent.FLAG_GRANT_READ_URI_PERMISSION)!=0)getContentResolver().takePersistableUriPermission(uri,Intent.FLAG_GRANT_READ_URI_PERMISSION);}catch(Exception ignored){}}fileCallback.onReceiveValue(result);fileCallback=null;enableImmersiveFullscreen();}}
