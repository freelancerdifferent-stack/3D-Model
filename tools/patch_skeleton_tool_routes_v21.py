...
route=r'''
 ...
 function applySkeletonFieldV21(id,value){if(!selectedBone)return;const n=Number(value);if(!Number.isFinite(n))return;if(id==='px')selectedBone.position.x=n;else if(id==='py')selectedBone.position.y=n;else if(id==='pz')selectedBone.position.z=n;else if(id==='rx')selectedBone.rotation.x=THREE.MathUtils.degToRad(n);else if(id==='ry')selectedBone.rotation.y=THREE.MathUtils.degToRad(n);else if(id==='rz')selectedBone.rotation.z=THREE.MathUtils.degToRad(n);else if(id==='sx')selectedBone.scale.x=n;else if(id==='sy')selectedBone.scale.y=n;else if(id==='sz')selectedBone.scale.z=n;selectedBone.updateMatrixWorld(true);if(skeletonHelper)skeletonHelper.update();syncSkeletonTransformFieldsV21()}
