...
new_touch="""let touchId=null,startX=0,startY=0,startPos=null,startQuat=null,startScale=null;\n let skeletonGestureBeforeV21=null;\n const skeletonUndoV21=[];\n const skeletonRedoV21=[];\n function boneStateV21(b){return b?{bone:b,pos:b.position.clone(),quat:b.quaternion.clone(),scale:b.scale.clone()}:null}\n function cloneBoneStateV21(st){return st?{bone:st.bone,pos:st.pos.clone(),quat:st.quat.clone(),scale:st.scale.clone()}:null}\n function cloneSkeletonHistoryV21(h){return h?{before:cloneBoneStateV21(h.before),after:cloneBoneStateV21(h.after)}:null}\n function sameBoneStateV21(a,b){if(!a||!b||a.bone!==b.bone)return false;return a.pos.distanceToSquared(b.pos)<1e-12&&1-Math.abs(a.quat.dot(b.quat))<1e-10&&a.scale.distanceToSquared(b.scale)<1e-12}\n function applyBoneStateV21(st){if(!st?.bone)return;setSkeletonSelectedBoneV21(st.bone);st.bone.position.copy(st.pos);st.bone.quaternion.copy(st.quat);st.bone.scale.copy(st.scale);st.bone.updateMatrixWorld(true);if(skeletonHelper)skeletonHelper.update();syncSkeletonTransformFieldsV21()}\n function pushSkeletonHistoryV21(before,after){if(!before||!after||sameBoneStateV21(before,after))return;skeletonUndoV21.push({before:cloneBoneStateV21(before),after:cloneBoneStateV21(after)});if(skeletonUndoV21.length>100)skeletonUndoV21.shift();skeletonRedoV21.length=0;syncSkeletonUndoButtonsV21()}\n function skeletonUndoActionV21(){const h=skeletonUndoV21.pop();if(!h){msg('Skeleton: tidak ada Undo');return}const entry=cloneSkeletonHistoryV21(h);applyBoneStateV21(entry.before);skeletonRedoV21.push(entry);syncSkeletonUndoButtonsV21();msg('Skeleton Undo')}\n function skeletonRedoActionV21(){const h=skeletonRedoV21.pop();if(!h){msg('Skeleton: tidak ada Redo');return}const entry=cloneSkeletonHistoryV21(h);applyBoneStateV21(entry.after);skeletonUndoV21.push(entry);syncSkeletonUndoButtonsV21();msg('Skeleton Redo')}"""
...
route=r'''
 ...
 function syncSkeletonUndoButtonsV21(){if(!skeletonLiveEditMode)return;if(undoBtnV21){undoBtnV21.disabled=false;undoBtnV21.style.opacity=skeletonUndoV21.length?'1':'.38'}if(redoBtnV21){redoBtnV21.disabled=false;redoBtnV21.style.opacity=skeletonRedoV21.length?'1':'.38'}}
 ...
 document.addEventListener('click',ev=>{if(!skeletonLiveEditMode)return;const b=ev.target.closest('button');if(!b)return;if(b===transformBtnV21){ev.preventDefault();ev.stopPropagation();ev.stopImmediatePropagation();openSkeletonTransformV21();return}if(b===undoBtnV21){ev.preventDefault();ev.stopPropagation();ev.stopImmediatePropagation();skeletonUndoActionV21();return}if(b===redoBtnV21){ev.preventDefault();ev.stopPropagation();ev.stopImmediatePropagation();skeletonRedoActionV21();return}if(b.classList.contains('tool')){const t=(b.textContent||'').toLowerCase();if(t.includes('move')||t.includes('rotate')||t.includes('scale')){ev.preventDefault();ev.stopPropagation();ev.stopImmediatePropagation();skeletonTransformMode=t.includes('move')?'move':(t.includes('scale')?'scale':'rotate');rail.querySelectorAll('.tool').forEach(x=>x.classList.remove('active'));b.classList.add('active');const badge=$('liveEditBadge');if(badge)badge.textContent='LIVE EDIT Skeleton • '+skeletonTransformMode.toUpperCase();return}}},true);
 ...
 const oldSkeletonModeClickV21=skeletonModeBtn.onclick;skeletonModeBtn.onclick=()=>{oldSkeletonModeClickV21?.();if(skeletonLiveEditMode){syncSkeletonUndoButtonsV21();syncSkeletonTransformFieldsV21()}else{if(undoBtnV21){undoBtnV21.disabled=false;undoBtnV21.style.opacity=''}if(redoBtnV21){redoBtnV21.disabled=false;redoBtnV21.style.opacity=''}}};
'''
